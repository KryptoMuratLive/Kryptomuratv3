...
const MetaverseWorld: React.FC<MetaverseProps> = ({ walletAddress, isConnected, apiBase }) => {
  ...
  const [userGallery, setUserGallery] = useState<UserGallery | null>(null);
  ...
  const loadWorldData = async () => {
    try {
      const response = await axios.get(`${apiBase}/api/metaverse/world`);
  ...
  const loadAirdrops = async () => {
    try {
      const response = await axios.get(`${apiBase}/api/metaverse/airdrop/available`);
  ...
  const loadNFTs = async () => {
    try {
      const response = await axios.get(`${apiBase}/api/metaverse/marketplace/nfts`);
  ...
  const loadProposals = async () => {
    try {
      const response = await axios.get(`${apiBase}/api/metaverse/voting/proposals`);
  ...
  const loadUserGallery = async () => {
    ...
    try {
      const response = await axios.get(`${apiBase}/api/metaverse/gallery/${walletAddress}`);
  ...
  const claimAirdrop = async (airdropId: string) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${apiBase}/api/metaverse/airdrop/claim`, {
  ...
  const buyNFT = async (nftId: string) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${apiBase}/api/metaverse/marketplace/buy`, {
        wallet_address: walletAddress,
        nft_id: nftId
      });
  ...
  const castVote = async (proposalId: string, voteOption: string) => {
    ...
    try {
      setLoading(true);
      await axios.post(`${apiBase}/api/metaverse/voting/vote`, {
        wallet_address: walletAddress,
      ...
      <div className="p-4">
        ...
        <div className="container mx-auto">
          ...
          {activeArea === 'vip_lounge' && (
            <div className="text-center">
              <h2 className="text-4xl font-bold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400">
                👑 VIP Lounge
              </h2>
